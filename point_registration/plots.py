...
def create_lsa_mse_grouped_plots(
    ...
    title_raw = Path(output_filename_raw).stem
    title_transformed = Path(output_filename_transformed).stem
    
    # Separate data for highlighting specific responseids
    highlight_ids = ['R_0638fJAPpmzLtu1', 'R_3RxOa8kaiyul4bG']
    xy_flipped_ids = ['R_31bjqd6Mm8wBxN5']
    misnamed_ids = ['R_24Nwgngl83ucQ8B', 'R_6E6PxT3N1gr93yh']
    
    df_highlight = df_nuclei_analysis[df_nuclei_analysis['responseid'].isin(highlight_ids)]
    df_xy_flipped = df_nuclei_analysis[df_nuclei_analysis['responseid'].isin(xy_flipped_ids)]
    df_misnamed = df_nuclei_analysis[df_nuclei_analysis['responseid'].isin(misnamed_ids)]
    df_others = df_nuclei_analysis[~df_nuclei_analysis['responseid'].isin(highlight_ids + xy_flipped_ids + misnamed_ids)]
    
    # Step 6: Create the first plot - LSA MSE Raw
    print("Creating LSA MSE Raw plot...")
    fig_raw, ax_raw = plt.subplots(1, 1, figsize=figsize)
    
    sns.boxplot(data=df_nuclei_analysis, x='qx.2_clean', y='lsa_mse_raw', ax=ax_raw, color='lightblue', showfliers=False)
    # Plot other points in red
    sns.stripplot(data=df_others, x='qx.2_clean', y='lsa_mse_raw', ax=ax_raw, 
                  color='red', alpha=0.5, size=4, jitter=True)
    # Plot highlighted points in green
    if len(df_highlight) > 0:
        sns.stripplot(data=df_highlight, x='qx.2_clean', y='lsa_mse_raw', ax=ax_raw, 
                      color='green', alpha=0.8, size=6, jitter=True)
    # Plot xy flipped points in blue
    if len(df_xy_flipped) > 0:
        sns.stripplot(data=df_xy_flipped, x='qx.2_clean', y='lsa_mse_raw', ax=ax_raw, 
                      color='blue', alpha=0.8, size=6, jitter=True)
    # Plot misnamed points in orange
    if len(df_misnamed) > 0:
        sns.stripplot(data=df_misnamed, x='qx.2_clean', y='lsa_mse_raw', ax=ax_raw, 
                      color='orange', alpha=0.8, size=6, jitter=True)
    
    # Create custom legend
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor='red', alpha=0.5, label='Other responses')]
    if len(df_highlight) > 0:
        legend_elements.append(Patch(facecolor='green', alpha=0.8, label='Pixel to microns'))
    if len(df_xy_flipped) > 0:
        legend_elements.append(Patch(facecolor='blue', alpha=0.8, label='XY flipped'))
    if len(df_misnamed) > 0:
        legend_elements.append(Patch(facecolor='orange', alpha=0.8, label='Misnamed'))
    ax_raw.legend(handles=legend_elements, loc='best')
    
    ax_raw.set_yscale('log')
    ax_raw.set_title(title_raw, fontsize=14, fontweight='bold')
    ...
    sns.boxplot(data=df_nuclei_analysis, x='qx.2_clean', y='lsa_mse_transformed', ax=ax_transformed, color='lightblue', showfliers=False)
    # Plot other points in red
    sns.stripplot(data=df_others, x='qx.2_clean', y='lsa_mse_transformed', ax=ax_transformed, 
                  color='red', alpha=0.5, size=4, jitter=True)
    # Plot highlighted points in green
    if len(df_highlight) > 0:
        sns.stripplot(data=df_highlight, x='qx.2_clean', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='green', alpha=0.8, size=6, jitter=True)
    # Plot xy flipped points in blue
    if len(df_xy_flipped) > 0:
        sns.stripplot(data=df_xy_flipped, x='qx.2_clean', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='blue', alpha=0.8, size=6, jitter=True)
    # Plot misnamed points in orange
    if len(df_misnamed) > 0:
        sns.stripplot(data=df_misnamed, x='qx.2_clean', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='orange', alpha=0.8, size=6, jitter=True)
    ...
    if len(df_misnamed) > 0:
        legend_elements.append(Patch(facecolor='orange', alpha=0.8, label='Misnamed'))
    ax_transformed.legend(handles=legend_elements, loc='best')
    
    ax_transformed.set_yscale('log')
    ax_transformed.set_title(title_transformed, fontsize=14, fontweight='bold')
...
def create_lsa_mse_software_grouped_plots(
    registration_stats_file='registration_stats.csv',
    combined_data_file='../data_analysis/combined data deidentified.csv',
    output_filename_raw='LSA MSE Raw - Grouped by QX.2 - grouped.png',
    output_filename_transformed='LSA MSE Transformed - Grouped by QX.2 - grouped.png',
    figsize=(10, 8)
):
    """
    Create and save two separate LSA MSE plots grouped by software type for nuclei data.
    
    This function groups software into two categories:
    - Commercial: Imaris, Arivis, Aivia
    - Open Source: Python, CellProfiler Python, Fiji/ImageJ
    ...
    combined_data_file : str
        Path to the combined data deidentified CSV file
    output_filename_raw : str
        Filename to save the raw MSE plot
    output_filename_transformed : str
        Filename to save the transformed MSE plot
    figsize : tuple
        Figure size (width, height) in inches for each individual plot
    ...
    # Step 6: Create software type grouping
    print("Creating software type groups...")
    commercial_software = ['Imaris', 'Arivis', 'Aivia']
    open_source_software = ['Python', 'Cell Profiler', 'Cell Profiler,Python', 'FIJI/ImageJ']
    
    def map_to_software_group(software_name):
        if pd.isna(software_name):
            return None
        software_str = str(software_name).strip()
        if software_str in commercial_software:
            return 'Imaris/Arivis/Aivia'
        elif software_str in open_source_software:
            return 'Python/ImageJ/CellProfiler'
        else:
            return None
    
    df_nuclei_analysis['software_group'] = df_nuclei_analysis['qx.2_clean'].apply(map_to_software_group)
    df_nuclei_analysis_grouped = df_nuclei_analysis.dropna(subset=['software_group'])
    
    print(f"Grouped nuclei rows: {len(df_nuclei_analysis_grouped)}")
    ...
    df_highlight = df_nuclei_analysis_grouped[df_nuclei_analysis_grouped['responseid'].isin(highlight_ids)]
    df_xy_flipped = df_nuclei_analysis_grouped[df_nuclei_analysis_grouped['responseid'].isin(xy_flipped_ids)]
    df_misnamed = df_nuclei_analysis_grouped[df_nuclei_analysis_grouped['responseid'].isin(misnamed_ids)]
    df_others = df_nuclei_analysis_grouped[~df_nuclei_analysis_grouped['responseid'].isin(highlight_ids + xy_flipped_ids + misnamed_ids)]
    
    # Step 7: Create the first plot - LSA MSE Raw
    print("Creating LSA MSE Raw grouped plot...")
    fig_raw, ax_raw = plt.subplots(1, 1, figsize=figsize)
    
    sns.boxplot(data=df_nuclei_analysis_grouped, x='software_group', y='lsa_mse_raw', ax=ax_raw, color='lightblue', showfliers=False)
    # Plot other points in red
    sns.stripplot(data=df_others, x='software_group', y='lsa_mse_raw', ax=ax_raw, 
                  color='red', alpha=0.5, size=4, jitter=True)
    # Plot highlighted points in green
    if len(df_highlight) > 0:
        sns.stripplot(data=df_highlight, x='software_group', y='lsa_mse_raw', ax=ax_raw, 
                      color='green', alpha=0.8, size=6, jitter=True)
    # Plot xy flipped points in blue
    if len(df_xy_flipped) > 0:
        sns.stripplot(data=df_xy_flipped, x='software_group', y='lsa_mse_raw', ax=ax_raw, 
                      color='blue', alpha=0.8, size=6, jitter=True)
    # Plot misnamed points in orange
    if len(df_misnamed) > 0:
        sns.stripplot(data=df_misnamed, x='software_group', y='lsa_mse_raw', ax=ax_raw, 
                      color='orange', alpha=0.8, size=6, jitter=True)
    ...
    ax_raw.set_yscale('log')
    ax_raw.set_title(title_raw, fontsize=14, fontweight='bold')
    ax_raw.set_xlabel('Software Type', fontsize=12)
    ax_raw.set_ylabel('LSA MSE Raw (um²) - Log Scale', fontsize=12)
    ax_raw.grid(True, alpha=0.3, which='both')
    ax_raw.tick_params(axis='x', rotation=0)
    
    plt.tight_layout()
    
    # Save the raw plot in figures directory
    raw_plot_path = figures_dir / output_filename_raw
    print(f"Saving raw grouped plot to {raw_plot_path}...")
    fig_raw.savefig(raw_plot_path, dpi=300, bbox_inches='tight')
    print("Raw grouped plot saved successfully!")
    
    # Step 8: Create the second plot - LSA MSE Transformed
    print("Creating LSA MSE Transformed grouped plot...")
    fig_transformed, ax_transformed = plt.subplots(1, 1, figsize=figsize)
    
    sns.boxplot(data=df_nuclei_analysis_grouped, x='software_group', y='lsa_mse_transformed', ax=ax_transformed, color='lightblue', showfliers=False)
    # Plot other points in red
    sns.stripplot(data=df_others, x='software_group', y='lsa_mse_transformed', ax=ax_transformed, 
                  color='red', alpha=0.5, size=4, jitter=True)
    # Plot highlighted points in green
    if len(df_highlight) > 0:
        sns.stripplot(data=df_highlight, x='software_group', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='green', alpha=0.8, size=6, jitter=True)
    # Plot xy flipped points in blue
    if len(df_xy_flipped) > 0:
        sns.stripplot(data=df_xy_flipped, x='software_group', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='blue', alpha=0.8, size=6, jitter=True)
    # Plot misnamed points in orange
    if len(df_misnamed) > 0:
        sns.stripplot(data=df_misnamed, x='software_group', y='lsa_mse_transformed', ax=ax_transformed, 
                      color='orange', alpha=0.8, size=6, jitter=True)
    ...
    ax_transformed.set_yscale('log')
    ax_transformed.set_title(title_transformed, fontsize=14, fontweight='bold')
    ax_transformed.set_xlabel('Software Type', fontsize=12)
    ax_transformed.set_ylabel('LSA MSE Transformed (um²) - Log Scale', fontsize=12)
    ax_transformed.grid(True, alpha=0.3, which='both')
    ax_transformed.tick_params(axis='x', rotation=0)
    
    plt.tight_layout()
    
    # Save the transformed plot in figures directory
    transformed_plot_path = figures_dir / output_filename_transformed
    print(f"Saving transformed grouped plot to {transformed_plot_path}...")
    fig_transformed.savefig(transformed_plot_path, dpi=300, bbox_inches='tight')
    print("Transformed grouped plot saved successfully!")
    
    # Print summary statistics
    print("\n" + "="*60)
    print("Summary Statistics by Software Type")
    print("="*60)
    for software_group in sorted(df_nuclei_analysis_grouped['software_group'].dropna().unique()):
        group_data = df_nuclei_analysis_grouped[df_nuclei_analysis_grouped['software_group'] == software_group]
        print(f"\nSoftware Type = {software_group} (n={len(group_data)})")
        print(f"  LSA MSE Raw - Mean: {group_data['lsa_mse_raw'].mean():.4f}, Median: {group_data['lsa_mse_raw'].median():.4f}")
        print(f"  LSA MSE Transformed - Mean: {group_data['lsa_mse_transformed'].mean():.4f}, Median: {group_data['lsa_mse_transformed'].median():.4f}")
    
    return fig_raw, fig_transformed, df_nuclei_analysis_grouped


if __name__ == '__main__':
    # Example usage
    fig_raw, fig_transformed, df_nuclei = create_lsa_mse_grouped_plots()
    fig_raw_grouped, fig_transformed_grouped, df_nuclei_grouped = create_lsa_mse_software_grouped_plots()
    plt.show()
